...
var cssTasks = function(filename) {
  return lazypipe()
    ...
    .pipe($.autoprefixer, {
      browsers: [
        'last 2 versions', 
        'ie 8', 'ie 9', 'android 2.3', 'android 4', 'opera 12'
      ]
    })
    .pipe($.minifyCss)
    .pipe(function() {
...
var cssTasksDev = function(filename) {
  return lazypipe()
    .pipe(function() {
      return $.if('*.less', $.less());
    })
...
var jsTasksDev = function(filename) {
  return lazypipe()
    .pipe($.concat, filename)();
};
...
gulp.task('styles-dev', function() {
  var merged = merge();
  manifest.forEachDependency('css', function(dep) {
    var cssTasksInstance = cssTasksDev(dep.name);
    if (!enabled.failStyleTask) {
      cssTasksInstance.on('error', function(err) {
        console.error(err.message);
        this.emit('end');
...
gulp.task('watch', function() {
  ...
  gulp.watch([path.source + 'react/**/*'], ['react']);
...
gulp.task('watch-dev', function() {
  ...
  gulp.watch([path.source + 'styles/**/*'], ['styles-dev']);
  gulp.watch([path.source + 'scripts/**/*'], ['jshint', 'scripts-dev']);
  gulp.watch([path.source + 'react/**/*'], ['react']);
  gulp.watch(['bower.json', 'assets/manifest.json'], ['build']);
});
